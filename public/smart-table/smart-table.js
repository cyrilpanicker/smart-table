(function(angular){
    ...
    //     $templateCache.put('smart-table/smart-table.html','<div class="smart-table" ng-init="model=smartTableModel"> <div class="top-pagination" ng-if="ngTableParamsObject.$params.paginate" ng-include="\'smart-table/smart-table-pagination.html\'" > </div><table ng-table="ngTableParamsObject"> <thead> <tr> <th class="row-select" ng-show="params.$params.rowSelectable" ng-style="{\'width\':params.$params.rowSelectorColumnWidth+\'%\'}"> <input id="selectAllCheckBox" type="checkbox" ng-checked="model.allRowsSelected" ng-click="model.onSelectAllClick($event)"/> </th> <th ng-repeat="col in params.$params.columns" ng-click="col.sortable && params.serverSortBy(col.field)" ng-class="{\'sortable\':col.sortable, \'sort-asc\':model.sortParams.sortColumn===col.field && model.sortParams.sortOrder===\'asc\', \'sort-desc\':model.sortParams.sortColumn===col.field && model.sortParams.sortOrder===\'desc\'}" ng-style="{\'width\':col.width+\'%\'}" > <div ng-bind="col.title"></div></th> <th ng-show="params.$params.infoColumns.length" ng-style="{\'width\':params.$params.infoColumnWidth+\'%\'}"> </th> <th ng-repeat="col in params.$params.actionColumns" ng-style="{\'width\':col.width+\'%\'}"> <div ng-bind="col.title"></div></th> </tr></thead> <tbody> <tr ng-repeat="datum in $data" ng-show="!model.loading && $data.length"> <td class="row-select" ng-show="params.$params.rowSelectable"> <input type="checkbox" ng-model="datum._isSelected" ng-click="model.updateSelectedRows()"/> </td><td ng-repeat="col in params.$params.columns" ng-bind="!datum[col.field] ? ((datum[col.field]===0||datum[col.field]===false) ? datum[col.field] : col.defaultText) : datum[col.field]" > </td><td class="info-column" ng-show="params.$params.infoColumns.length"> <div smart-table-tooltip-wrapper class="tooltip-wrapper left"> <a href="" class="info-icon"></a> <div class="tooltip-container"> <div class="tooltip-content"> <span ng-repeat="col in params.$params.infoColumns"> <span class="title" ng-bind="col.title"></span> : <span ng-bind="!datum[col.field] ? ((datum[col.field]===0||datum[col.field]===false) ? datum[col.field] : col.defaultText) : datum[col.field]"></span> <br/> </span> </div></div></div></td><td class="action-column" ng-repeat="col in params.$params.actionColumns"> <a ng-repeat="action in col.actions" ng-class="{\'image-link\':action.imageUrl}" href="" ng-click="model.onAction(action.id,datum)"> <span ng-show="!action.imageUrl" ng-bind="action.text"></span> <img ng-show="action.imageUrl" ng-src="{{action.imageUrl}}" ng-attr-title="{{action.text}}"/> </a> </td></tr><tr class="no-records" ng-show="!model.loading && !$data.length"> <td colspan="{{params.$params.columns.length + params.$params.actionColumns.length + params.$params.rowSelectable + !!params.$params.infoColumns.length}}" ng-bind="model.noRecordsMessage" > </td></tr><tr class="loading-message" ng-show="model.loading"> <td colspan="{{params.$params.columns.length + params.$params.actionColumns.length + params.$params.rowSelectable + !!params.$params.infoColumns.length}}" ng-bind="model.loadingMessage" > </td></tr></tbody> </table> <div class="bottom-pagination" ng-if="ngTableParamsObject.$params.paginate" ng-include="\'smart-table/smart-table-pagination.html\'" > </div></div>');
    // }])
